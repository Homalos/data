...
```
DEBUG    | [trace_id=req-12345] [database] 查询用户: 123
INFO     | [trace_id=req-12345] [request] 处理请求
SUCCESS  | [trace_id=req-12345] [request] 订单处理完成
WARNING  | [trace_id=req-12345] [auth] 密码错误: user123
ERROR    | [trace_id=req-12345] [connection] WebSocket 连接异常
```
...
# SUCCESS - 操作成功
logger.success("订单创建成功", tag="order")
...
A: 不是必需的。Tag 是可选的，当不需要分类时可以不指定。

### Q: 如何修改日志级别？

A: 编辑 `src/utils/log/logger.py` 中的 `level` 参数。
